...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; function Start-Process { [CmdletBinding()] param([Parameter(Mandatory=$true,Position=0)][string]$FilePath,[Parameter(Position=1)][string[]]$ArgumentList,[string]$WorkingDirectory,[string]$RedirectStandardOutput,[string]$RedirectStandardError,[string]$WindowStyle,[switch]$PassThru); $parameters=@{FilePath=$FilePath}; if ($PSBoundParameters.ContainsKey('ArgumentList')) { $quote=[string][char]34; $parameters.ArgumentList=(@($ArgumentList | ForEach-Object { $quote + $_.Replace($quote,('\'+$quote)) + $quote }) -join ' ') }; foreach ($name in @('WorkingDirectory','RedirectStandardOutput','RedirectStandardError','WindowStyle')) { if ($PSBoundParameters.ContainsKey($name)) { $parameters[$name]=Get-Variable -Name $name -ValueOnly } }; if ($PassThru) { $parameters.PassThru=$true }; Microsoft.PowerShell.Management\Start-Process @parameters }; $root=[IO.Path]::GetFullPath($env:LB_REPO_ROOT); $scriptPath=$env:LB_START_SCRIPT; Set-Location -LiteralPath $root; $statusOutput=& $scriptPath -Action Status; if (-not $?) { throw 'scripts\\start-source.ps1 -Action Status failed.' }; $status=$statusOutput | ConvertFrom-Json; if ($status.status -ne 'running') { & $scriptPath -Action Start; if (-not $?) { throw 'scripts\\start-source.ps1 failed.' } }"
if errorlevel 1 goto :failed
