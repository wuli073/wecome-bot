...
export const CONVERSATION_SEARCH_DELAY_MS = 800
export const CONVERSATION_INPUT_DELAY_MS = 800
...
export async function prepareDraftInput(
  ...
  try {
    ...
    recordPhase('conversation_name_pasted')

    stage = 'waiting_for_conversation_search'
    recordPhase('conversation_search_waiting')
    await sleep(CONVERSATION_SEARCH_DELAY_MS)
    throwIfCancelled(isCancelled)
    ...
    recordPhase('conversation_enter_sent')

    stage = 'waiting_for_conversation_input'
    recordPhase('conversation_input_waiting')
    await sleep(CONVERSATION_INPUT_DELAY_MS)
    throwIfCancelled(isCancelled)
    await ensureWindowStillActive(getActiveWindow, activeWindow, 'SEARCH_RESULT_CONFIRM_FAILED')

    stage = 'clearing_conversation_input'
    throwIfCancelled(isCancelled)
    await deps.input.hotkey(['Control', 'A'])
    recordPhase('conversation_input_cleared')
...
function classifyPasteOnlyFailure(stage: string, error: unknown): string {
  ...
  if (stage === 'pasting_conversation_name' || stage === 'waiting_for_conversation_search') return 'CONVERSATION_NAME_INPUT_FAILED'
  if (stage === 'confirming_search_result' || stage === 'waiting_for_conversation_input') return 'SEARCH_RESULT_CONFIRM_FAILED'
  if (stage === 'clearing_conversation_input' || stage === 'pasting_text') return 'TEXT_PASTE_FAILED'
